...
%\answer
\begin{itemize} \tightlist
    \item It's the opposite linearly.
    \item It's the same in terms of scope (hierarchically).
\end{itemize}

\paragraph{Summary} Auxiliaries have a rigid ordering which reflects scope, and that these auxiliaries might be independent syntactic/phonological words in one language but affixes in another.
...
% (Why do we gloss these sometimes as words and sometimes as affixes? Usually phonology.)
...
\pex[exno=11]
    \a \begingl
        ...
        \glft `It stopped snowing at night.' (does not snow at night anymore)// %\answer
    \endgl
    \a \begingl
        \gla nipaa-ishkwaa-sookihpwan//
        \glb at.night-finish-be.snowing//
        \glft `It stopped snowing at night.' (was snowing the whole day)// %\answer
    \endgl
\xe
\pex[exno=11']
    \a \begingl
        ...
        \glft `He secretly eats fast.' (nobody knows that he eats fast)// %\answer
    \endgl
    \a \begingl
        \gla kishahtapi-kiimooci-wiihsini//
        \glb fast-secretly-eat//
        \glft `He eats secretly (nobody knows that he eats) and he does it fast.'// %\answer
    \endgl
\xe
...
\paragraph{Summary} The order of affixes reflects the order of syntactic heads, which reflects semantic scope; this is the Mirror Principle \citep{baker85}.
...
Note how arguments (DPs, phrases) and suffixes live in harmony in the same tree structure.
...
\pex
    \a \emph{unlockable, unfoldable}, \dots{} \hfill \citep{dealmedialibben05,viknervikner08,pollatseketal10}
...
\bigskip
%\answer
\paragraph{Generalization.} The semantic relationship between the verb and the internal argument is less predictable than that between the verb and the external argument, which is predictable.
...
\ex \begingl
    ...
    \glft a. `John did not make Mary run.' (= she ran on her own accord)\\
    b. `John made Mary not run.' (= he prevented her from running)//
    \endgl
\xe
