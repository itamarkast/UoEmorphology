...
\pex
    ...
    \vspace{5cm}
    % like all our idiosyncratic verbs, might be the result of adjoining \gsc{CAUS} to the root:\\
        % \Tree
            % [.
            %     [.\root{\gsc{smell}} ]
            %     [.{\gsc{CAUS}\\\emph{sase}} ]
            % ]
    \a An analytical causative where the causee is the patient
    \vspace{5cm}
    % might only take the vP, so the argument remains the patient:\\
    %     \Tree
    %     [.
    %         [.{DP1\\Agent\\Causer} ]
    %         [.
    ...
    %         ]
    %     ]
    \a An analytical causative where the causee is the agent
    \vspace{5cm}
    % might take a full VoiceP:\\
    %     \Tree
    %     [.
    %         [.{DP1\\Agent\\Causer} ]
    %         [.
...
\ex \begingl
    ...
    Context: \vspace{2cm} \\
    % It is sports day at school. Mary wanted to play volleyball in the morning, but the teacher made her run instead. In the afternoon the teacher made Mary run again.\\
    b. \gsc{CAUS} > \emph{again} V\\
    Context: \vspace{2cm} //
    % Mary ran around the field in the morning, but the teacher wasn't watching. So in the afternoon the teacher made Mary run again.//
    \endgl
\xe
...
\ex \begingl
    ...
    \glft a. \longline{}\\ %`John did not make Mary run.' (= she ran on her own accord)\\
    b. \longline{} // %`John made Mary not run.' (= he prevented her from running)//
    \endgl
\xe
\bigskip
...
\pex \begingl
    ...
    Context: \vspace{1cm}
    % We are at a dinner party at Ay\c{s}e's house. Ay\c{s}e’s friend Mary is bored. Ay\c{s}e’s mother quietly asks Ay\c{s}e to make Mary play with her toys.
    \a \gsc{CAUS} > \emph{quietly} \gsc{CAUS} > V\\
    Context: \vspace{1cm}
    % Ay\c{s}e’s friend Mary is bored in the next room. Ay\c{s}e’s mother asks Ay\c{s}e to quietly go and make Mary play with her toys.
    \a \ljudge{\#} \gsc{CAUS} > \gsc{CAUS} > \emph{quietly} V\\
    Context: \vspace{1cm} 
    % Ay\c{s}e’s friend Mary is playing loudly with her toys. Ay\c{s}e’s mother asks Ay\c{s}e to make Mary play quietly.
\xe
...
The remainder of this section gives examples of other processes which add or reduce the number of arguments in a clause.
