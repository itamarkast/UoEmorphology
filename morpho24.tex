...
\pex
    \a Morphology isn't a separate module from syntax; words are built up in syntactic trees (because of scope, which works the same way in words and in sentences, i.e.~the mirror principle)
    \a A morpheme is a node in the tree (because it keeps things tidy)
    ...
    \a A lexical root is notated with the \root{root} symbol (because that makes it easy to work with bound roots like \root{etern})
    \a All roots need to be categorized as a noun, verb or adjective
...
\ex Structure: \inSolution{ {[[[} Adjective ] Comparative ] Superlative ] }
\xe
