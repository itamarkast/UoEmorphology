...
\newcommand\reponse[1]{\textcolor{blue}{#1}}
...
Ave les adjectifs qui ont un comparatif irrégulier comme \textit{bon}, la Condition Ailleurs (\textit{Elsewhere condition}) fait gagner l'allomorphe irrégulier sur la forme régulière:
...
Observez maintenant ces mêmes généralisations en vous mettant dans la peau d'un morphologue constructionnel (comme ceux qui font de la morphologie distribuée) et qui adoptent l'idée que la structure morphologique reflète directement la structure syntactico-sémantique.
...
Rappelons deux hypothèses de la morphologie distribuée:
\begin{enumerate}
    \item Condition Ailleurs/principe de Pañini: si deux règles sont en compétition pour l'épelation, alors la règle la plus spécifique s'applique d'abord (\textit{mieux} `gagne sur' \textit{plus bon}).
        \item Principe du miroir: La structure morphologique reflète la structure syntactico-sémantique
        \end{enumerate}
   
        
  \reponse{Ajoutons à ces deux hypothèses l'hypothèse suivante:}
        
        \begin{enumerate}
        \item[3.] \reponse{Le sens du superlatif `contient' le sens du comparatif: S3 = S2 + \( \gamma\)\\
    \( \to \) Vu le principe du miroir, la forme utilisée pour le superlatif doit contenir la forme utilisée pour le comparatif}
\end{enumerate}
\reponse{On prédit alors les données:
\begin{itemize}
