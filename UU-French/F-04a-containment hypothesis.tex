...
\begin{itemize} 
\item des relations implicatives entre les trois formes? Formulez
\item de l'inexistence des modèles non-attestés (*ABA/*AAB)?
\end{itemize}


\begin{answer}{
Relations implicatives entre les trois formes:
\begin{itemize}
...
Observez maintenant ces mêmes généralisations en vous mettant dans la peau d'un morphologue constructionnel (comme ceux qui font de la morphologie distribuée) et qui adoptent l'idée que la structure morphologique reflète directement la structure sémantique. 

Quelle hypothèse pourrait dans ce cadre expliquer ces généralisations? Supposez que 

\begin{itemize}
    \item S1 est le sens de l'adjectif;
    \item S2 est le sens du comparatif;
    \item S3 est le sens du superlatif.
\end{itemize}
...
\begin{answer}{Ajoutons à nos deux hypothèses précédentes (1) et (2) 
...
\begin{enumerate}
    ...
    \item Le sens du superlatif `contient' le sens du comparatif: S3 = S2 + \( \gamma\)\\
    \( \to \) La forme utilisée pour le superlatif doit contenir la forme utilisée pour le comparatif
    \item Principe du miroir: La structure morphologique reflète la structure sémantique
\end{enumerate}
On prédit alors les données:
\begin{itemize}
    \item Le patron AAA respecte les
    \item ABB: S1 + S2 (\textit{mieux}) + S3 (\textit{le mieux})
    \item *ABA: 
\end{itemize}
}
