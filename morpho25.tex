...
\hfill \emph{Morphology 2024-25, Edinburgh}
...
Once we have a hypothesis in place about how morphology works, we can make predictions about learning.
    ...
    \subsection{A theory}
The questions we ask will lead us to develop a variant of the framework called Distributed Morphology, or simply DM for short \citep{dm}. The core idea of DM is that morphology (words) have complex internal hierarchical structure just like syntax (sentences) does. DM actually assumes that morphology and syntax are one and the same structure. We will build up to this idea, seeing what it predicts and what we can learn along the way.
...
And some related questions: What elements can combine and how? What elements can stand on their own, and when? At what points in a ``word'' can we interrupt it, or interrupt the language user? What kinds of words are there?
...
\begin{enumerate} \tightlist
        ...
        \begin{itemize} \tightlist
            ...
            \item Constraint on phonological words in English: it has a minimal prosodic size
                \begin{itemize} \tightlist
                    ...
                    \item A distinction between tense/lax vowels, definable in terms of morae, which applies differently to lexical and functional items
                    \item If some varieties of the language allow for e.g.~\emph{awe} [ɔ], then we'd want to also investigate the vowel system of that variety in more depth
                    \item Some loanwords are borrowed with the original phonology, but then language users adapt the phonology to their L1: \emph{pho} can be original [fə] or adapted English-like [fo].
                    \item We can also see this with acronyms and whether they're usually pronounced as words or spelled out as individual letters:
                    \begin{itemize} \tightlist
                        \item LOL = /lɔl/ or L-O-L
                        \item LEL = /lɛl/ or L-E-L
                        \item CAHSS = /kas/
                        \item PPLS = P-P-L-S, not /ˈpˌpls/
                    \end{itemize}
                \end{itemize}
    ...
    \item Syntactic word
        \begin{itemize} \tightlist
            ...
            \item It's odd to say that \emph{will} and [PAST] are both the same kind of element
            \item This conceptualization doesn't give us a way of talking about compounds, or phrases
            \item Do words have their own internal structure? 
        \end{itemize}
    \item Psycholinguistic words
        \begin{itemize} \tightlist
            \item Not a definition so much as an approach to identify units that speakers might be sensitive to
...
We discussed the differences between inflection and derivation, and whether it's possible to postulate one basic difference which derives all the rest, or are we stuck with the notions of ``inflection'' and ``derivation'' as primitives of our theory, i.e.~something that we can't reduce to other factors. 
...
\begin{itemize} \tightlist
    ...
    \item A former government isn't *\emph{govern-ed-ment}, even though that could make semantic sense.
\end{itemize}
...
\begin{itemize} \tightlist
    ...
    \item In English, all prefixes appear to be derivational (and all inflectional affixes are suffixes).
\end{itemize}
...
As one student put it, there's many right ways to do a syntax tree but also many wrong ways. We want to focus on the scopal relationships between morphemes, rather than the formal specifics of what the best label or branching structure are.
}
\end{answer}
...
\ex[exno=4] \begingl
    ...
    \endgl \label{ex:alsina2}
\xe

\begin{answer}
{The order is whatever it needs to be to get scope right; you go from the verbal root ``outwards''. This is easier to see in \citeauthor{alsina99}'s~(3) than his~(4), which raises the question of whether the former is somehow more basic than the latter.}
\end{answer}
...
\inSolution{One way of thinking about this---suggested by a student on this course---is that scope is determined in terms of what's ``inside'' or ``outside'', while linearization (prefixes vs affixes) is in terms of ``right'' and ``left''.}
...
\begin{answer}
{
The first decision to make is what the `core' of our clause is: what takes lowest scope, and what scopes over it? There are a couple of reasons to think that for~(\blastx) it's `be.snowing'. First, its position is constant in both examples. Second, we don't know for sure that it's a verb, or that it's more of a main verb than `finish', but we do see it at the end, which is similar to some languages. And third, if we think that `finish' is the most deeply embedded element, then we run into trouble trying to come up with an interpretation for~(\blastx b) which is different from that of~(\blastx a). That said, both scopal orders are plausible hypotheses. We would need more information to know for certain. We also have some uncertainty around glosses like `finish' and how close they are to what we'd call \emph{finish} in English.
}
\end{answer}
...
\begin{itemize} \tightlist
    \item We start from `dog', which is intuitively the most deeply embedded part of the word.
    \item We're shifting from thinking about the linear order of affixes to thinking about their hierarchical (scopal) order.
    \item The question is then whether Number scopes over Grammatical Gender, or Gender over Number.
    \begin{itemize} \tightlist
        \item Morphologically, it's clear that Gender is closer to the verbal root, so Number scopes over it (the Mirror Principle).
        \item Semantically, we're dealing with a [group of [female dogs]], not a [female [group of dogs]].
    \end{itemize}
    \item The tree is ``schematic'' because we don't need to commit to the labels or anything like that: we can say F or Gen, PL or Num -- what we're focusing on right now is their hierarchical order.
    \item Put differently, the conceptual shift is in thinking about the internal structure of words.
\end{itemize}
}
\end{answer}
...
\end{itemize}
Let's see what \emph{novelizations} tells us:\\
\Tree
 [.[N]
    [.[N]
        [.[V]
...
Each morpheme carries out its usual job. The plural suffix at the end tells us that the resulting word is plural, but it's still a noun, so we can keep the overall label [N].
}
\end{answer}
...
\begin{answer}
{
There is no verb *\emph{unpredict} -- the intuition is that \emph{predict} first combines with \emph{able} to give us the adjective \emph{predictable}, and then the negative \emph{un-} \textbf{takes scope} over \emph{predictable} to give us \emph{un-predictable}. We can see the same thing happening with other derived forms of the same root, for example \emph{un-predicted}.
}
\end{answer}
...
\pex
    ...
    \inSolution{(Potentially \emph{bi-week-ly} and \emph{bi-month-ly}, though that might be lexical ambiguity)}
    \a Compounds like \fillblank{\emph{kitchen towel rack}}
\xe
\begin{answer}
{
The string in~(\blastx) is ambiguous between at least two readings: one in which I use a banana to hit the clown, and one in which the clown is holding a banana and I punch him. Unnecessary violence aside, we can understand this as syntactic or structural ambiguity:
    \pex[exno=27']
        \a {[}\textbf{hit} [the clown] \textbf{[with a banana]}]
        \a {[}hit [\textbf{the clown} [\textbf{with a banana}]]]
    \xe
The same happens with \emph{unfoldable}: a work of origami is [[un-fold]-able], because you can unfold it (\emph{able} takes scope over the verb \emph{unfold}). But you can't make origami out of something hard like a block of wood: it is [un-[fold-able]], where \emph{un-} takes scope over the adjective \emph{foldable}. This is also a case of structural ambiguity.
}
\end{answer}
...
\bigskip
Let's think back to inflection. If we assume that verbs always inflect for certain features in some language, then we often assume silent affixes (notated with the empty set symbol, \zero{}). This way the formal system is consistent (though not everyone is a fan of the concept of silent affixes; Paradigm Function Morphology for instance works very differently, \citealt{stump15}):
\pex
    \a she walk-s_{\gsc{3SG.PRES}}
    \a I walk-\zero{}_{\gsc{1.PRES}}
    \a They walk-\zero{}_{\gsc{PL.PRES}}
...
\bigskip
Returning to word-building (derivation), what about bound roots like ``\emph{etern}'', where we have \emph{etern-al} and \emph{etern-ity} but no standalone word *\emph{etern}? We'll need some way of representing the abstract, bound root. Sometimes people borrow the mathematical square-root symbol to show that the root needs some kind of affix to get its category:
\ex
\begin{minipage}[t]{0.4\textwidth}
    a.~\emph{eternity} \\
    \Tree
...
And if we think that \emph{some} words consist of a root and an affix providing the syntactic category, why not assume that all words are built this way? Then, for ``ordinary'' nouns like \emph{globe}:
\ex
\Tree
[.n
    [.\root{\gsc{globe}} ]
...
And once that's what we want to do, there's actually another choice point: now we would need grounds to decide which of the following two derivations to prefer for \emph{global} \citep{grestenbergerkastner22}, though there's currently no good answer there so I won't worry about that too much.
\begin{answer}{
\ex \emph{global}\\
    a.~
    \Tree
...
\bigskip
What does this mean crosslinguistically? We would expect to find languages in which all roots must be \emph{categorized} as a noun, verb or adjective first. Semitic languages seem to fit the bill.
...
Here are the assumptions we've made so far, step by step, together with the motivations for them.
\pex
    \a Morphology isn't necessarily a separate module from syntax; words are built up in hierarchical, i.e.~syntactic, trees (because of scope, which works the same way in words and in sentences, i.e.~the mirror principle).
    \a Syntactic hierarchy reflects semantic and morphological scope.
    \a A morpheme is a node in the tree (because that keeps things tidy).
    \a A complex word is composed of a root/stem and any number of categorizing affixes that attach above it (potentially changing its lexical category).
    \a We've created a new syntactic category (part of speech): the abstract root. A lexical root is notated with the \root{root} symbol (because that makes it easy to work with bound roots like \root{etern}).
    \a Roots can't be inserted into a clause on their own. They need to first combine with a categorizing head (like the verbalizer -\emph{ize}, the nominalizer -\emph{ity}, or silent counterparts) before that combined chunk can function as a noun, verb or adjective.
    \a Some syntactic elements are silent (because of how inflection works in languages like English).
    \a A categorizing affix might be silent.
\xe

        \subsubsection{Inflection and derivation}
...
Creation of new stems also makes sense: ``derivation'' is affixation of categorizing affixes, whereas ``inflection'' is selection of morphosyntactic features that are in a scopally higher position.
For regularity, we need to know which categorizing affix combines with which root/stem. But there's also a sense in which this is true for the allomorphy of inflection, which is our next topic.}
\end{answer}
...
\begin{answer}
{It looks like the first affix picks out one possible meaning, which is then consistent across further derivations.\\
The noun \emph{globe} has two main related meanings: a spherical ball, and the world. Once we derive \emph{glob-al}, only one of those meanings is kept (the international one): \emph{Taylor Swift is a global phenomenon}, but an origami piece crumpled into a ball is not \emph{global}. Once \emph{al-} attaches to the root and takes scope over it, that meaning persists in subsequent derivations: to roll origami into a globe shape is similarly not an act of \emph{globalization}. Similarly, to \emph{novelize} something means to turn a film into a book, but I can't create a \emph{novelization} of these lecture notes just by adding a story to them and printing them out.\\
\cite{arad03,arad05} makes a similar point for Hebrew, where the morphology shows us the same pattern in different ways.}
\end{answer}
...
What is allomorphy? What changes, and based on what? Let's find some examples first.
...
\ex Initial rules, to be revised when we get to *ABA:\\
    \gsc{GOOD} \lra $\begin{cases} 
...
The first pattern is one in which a higher element looks ``downards'' or ``inwards'' for the syntactic features of a ``lower'' or more deeply embedded element. Or, to use a different metaphor, the lower element conditions the higher element.

    \subsubsection{Bulgarian}
Assuming that we have the same root \root{gaz} in both~(\nextx) and~(\anextx), we see that the allomorphy of the determiner is conditioned by the gender of the noun; or by the number of the noun, as in~(\ref{ex:bulg-pl}).
\begin{multicols}{2}
\pex
    \a gaz\\
    gas.\gsc{MASC}\\
...
The definite article attaches after the noun is built, so it is sensitive inwards, to the syntactic feature on a lower head. This could be an independent Gender head, or we could put grammatical gender directly on the categorizer n \citep{kramer14,kramer15}.
\ex
    a. \Tree
    [.
        [.n 
...
Here's how one student visualized it:\\
\includegraphics[width=0.4\textwidth]{figs/freya.jpeg}
...
We've already seen this with \emph{go}$\sim$\emph{went}: the allomorphy of the root/verb depends on the tense value of the higher Tense head.
...
And also ellipsis as in~(\ref{ex:korean-sleep-ellipsis}).
        ...
        \subsubsection{Multiple affixes}
...
Itelmen \citep{bobaljik00}: class morpheme depends on Agr. \citet[10--11]{gouskovabobaljik20cup} show that this is about hierarchy, not linear order, because it holds for prefixes as well as suffixes:
...
\begin{enumerate} \tightlist
	\item Root \root{\gsc{pata}} `dog' with allomorphs \emph{pata} and \emph{der}.
...
Here's what English would look like if it had upward-looking phonological sensitivity:
\begin{enumerate} \tightlist
    \item Root \root{\gsc{PERSON}} with phonologically-conditioned allomorphs \emph{person} and \emph{people}.
    \item With C-initial suffix: \emph{person-ness}, \emph{person-s} (plural!).
    \item With V-initial suffix: \emph{people-ality}, \emph{people-age}.
\end{enumerate}
}
...
\begin{enumerate} \tightlist
    ...
    \end{itemize}
    \item Doesn't say anything about possible exceptions.
    \item Doesn't say anything about the relevant morphological/prosodic environments.
    \item This formalism lets us write anything!
...
\begin{enumerate} \tightlist
    ...
    \item Enforces binary branching.
    \item Potentially makes the hierarchical structure clearer than with phrase structure rules.
    \item Sets up a way of talking about movement in a more general form than the phrase structure grammar rules.
    \item Provides a way of talking about the difference between arguments and adjuncts (using the bar-level).
\end{enumerate}
}
...
\paragraph{Summary} These aren't the only relevant subfields, of course. But:
    \begin{itemize} \tightlist
        \item In each case we're setting up the architecture to tell us both what is and isn't possible.
        \item And then we can think about the origin of that architecture (be it cognitive, historical, etc).
    \end{itemize}
...
\begin{itemize} \tightlist
    ...
    \item Lexicalist theories assume that the syntax combines full words. Would that work?
        \begin{itemize} \tightlist
            \item Not immediately, because how would you know what word to put at the base of your tree if you don't have the rest of it with the conditioning information yet!
...
We now also have the information that \root{go} needs. So we can go ahead and choose its form, or \emph{exponent}, or \emph{vocabulary item}, namely \emph{went}. This is our syntactic conditioning (sensitive outwards):
\ex \Tree
[.
    [.
...
Or, alternatively, if we make an additional assumption that allows us to overwrite existing phonological material, then we might find ourselves in a loop whereby the two morphemes go back and forth changing their allomorphs based on each other's phonology.
...
\ex \label{ex:treat-nophono} \Tree
[.
    [.
...
The root doesn't care about T, so we fill in its phonology:
\ex \label{ex:treat-phono1} \Tree
[.
    [.
...
A major topic of research in allomorphy concerns \emph{locality}: how ``close'' do morphemes need to be in order to trigger allomorphy? What are the relevant hierarchical or linear constraints? We've discussed the hieararchical considerations arising from one case, *ABA, but there is much more to say. You'll find some pointers in the rest of this section.

        \subsubsection{*ABA}
[Exercise 7]

We're adding a layer of complexity to our previous discussion by considering two things simultaneously: suppletion of the adjectival root, and the hierarchical relations between two additional heads.
...
We need to decide about the structure of the root/adjective, comparative and superlative morphemes -- what combines with what? What takes scope over what?
...
With [[[Adj] \gsc{CMPR}] \gsc{SPRL}], the following options are possible:

\begin{tabular}{ccc}
    [Adj] & [[Adj] \gsc{CMPR}] & [[[Adj] \gsc{CMPR}] \gsc{SPRL}]\\\hline
    wug & wug-er & wug-est \\
    A   & A &   A \\\hline
        & baz-er & baz-est \\
        & B & B \\\hline
...
So the possible patterns are: AAA, ABB, ABC, AAB.

\bigskip
On the other hand, if we'd had [[Adj] \gsc{SPRL}], then the following options would be possible:

\begin{tabular}{ccc}
    [Adj] & [[Adj] \gsc{CMPR}] & [[Adj] \gsc{SPRL}]\\\hline
    wug & wug-er & wug-est \\
    A   & A &   A \\\hline
        & baz-er & \\
        & B & \\\hline
...
So the possible patterns are: AAA, \textbf{ABA}, AAC (AAB) and ABC. ABB would only be possible if the allomorphs of the adjective just happened to be B for both the comparative and the superlative, as a kind of coincidence.

\paragraph{Beyond superlatives.} Here's one nice example of how the *ABA story extends further. Many languages can derive change-of-state verbs from adjectives. When we do that, notice what form of the root we get:

\ex I heated/cooled the soup.
\xe
\pex
    \a The latest developments have worsened/*baddened/*badded/*worstened the situation.
    \a The recent agreement aims to better/*gooden/*engooden/*good/*best our working conditions.
    \a Student numbers have lessend/*fewered/*fewened/*fewed in recent years.
\xe

When the adjective has a suppletive form, it's the comparative form (rather than the positive or superlative one) that's used as the base for the verb! This makes sense if we think about what comparatives are: they denote some movement on a scale, but not necessarily movement all the way:
\ex The soup cooled (but it's still too hot to eat).
\xe
\ex The situation has worsened (but it's still alright).
\xe
\ex Eating healthily lessens the risks of illness (but doesn't remove them).
\xe

We'll return to this in the lexical semantics block.

\paragraph{Further reading:} For deriving *AAB and many other *ABA phenomena outwith adjectival suppletion, see \cite{bobaljik12,bobaljik15ar}, \cite{smithetal16supplet}, \cite{mueller20}, and the articles in \cite{cahavanden17}. On the differences between comparatives with -\emph{er} and comparatives with \emph{more}, see \cite{embickmarantz08} and \cite{kastner19cup}.

        \subsubsection{Locality in DM: \cite{arad03}}
...
\textbf{Observation:} \inSolution{Verbs/\textbf{Predicates} (in English) require at most one \textbf{subject} (external argument), one \textbf{direct object} (internal argument) and one \textbf{indirect/applied/oblique} argument at the same time.}
...
\begin{answer}{
It's the same event, except with a different subject/agent. Even the computer example makes sense if we think of the ``sleep/hybernate'' function.
}\end{answer}
...
\begin{itemize} \tightlist
    ...
    \item We assume, then, that we can't just merge arguments whenever we want; some functional head needs to introduce them (v for internal arguments, Voice for external arguments, and maybe others for other kinds of arguments like indirect objects).
\end{itemize}
}
...
Here's another example with its associated readings (or rather, the situations that can be plausibly expressed using this sentence), suggested by a student on the course:
\pex Sam cooled the milk again.
    \a Repetitive \#1, event + agent: \inSolution{\textbf{Sam} cooled the milk. Then the milk got warm. Then Sam cooled the milk a second time. So Sam had cooled it the first time, and then Sam cooled it again.}
    \a Repetitive \#2, event: \inSolution{\uline{Alex} cooled the milk. Then the milk got warm. Then Sam cooled the milk. So Alex had cooled it the first time, and then Sam cooled it again.}
    \a Restitutive, state: \inSolution{the milk was already cold (or not warm). Then it got warm. Then Sam cooled it. So Sam made it cool again, but no one had cooled it to make it cool the first time around.}
\xe
